...
/**
 * Unit tests for {@link RestAccessDeniedHandler}.
 */
public class RestAccessDeniedHandlerTest {

    @Test
    public void handle_ShouldReturn403WithJsonBodyAndStandardFields() throws IOException, ServletException {
        ...
        AccessDeniedException accessDeniedException = new AccessDeniedException("Required role ADMIN") {
            private static final long serialVersionUID = 1L;
        };

        handler.handle(request, response, accessDeniedException);

        Assertions.assertEquals(HttpStatus.FORBIDDEN.value(), response.getStatus(), "HTTP status should be 403");
        ...
        Assertions.assertEquals(HttpStatus.FORBIDDEN.value(), ((Number) body.get("status")).intValue(), "Status field should be 403");
        Assertions.assertEquals(HttpStatus.FORBIDDEN.getReasonPhrase(), body.get("error"), "Error field should match reason phrase");
        Assertions.assertEquals("/api/admin", body.get("path"), "Path field should match request URI");
    ...
    @Test
    public void handle_ShouldFallbackToDefaultMessageWhenExceptionIsNull() throws IOException, ServletException {
        ObjectMapper objectMapper = new ObjectMapper();
        RestAccessDeniedHandler handler = new RestAccessDeniedHandler(objectMapper);
